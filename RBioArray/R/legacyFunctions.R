...
#'
#' @description (Legacy function) Import GTF/GFF files. Use the new and much improved function \code{\link{rbioseq_gtf}} instead.
...
rbioseq_import_gtf <- function(file,
  ...
  if (any(class(filecheck) == "try-error")) {
    ...
    for (i in seq(length(tmpfile))) {
      ...
      tmp9 <- tmp9[c(1, 2, 3, 5)]  # only the first 8 are useful
